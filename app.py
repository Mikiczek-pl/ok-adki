...
# WAŻNE: zgodnie z Twoją prośbą -> ROZCIĄGANIE do 130x150 (bez kadrowania)
...
def wrap_text_by_pixels(draw: ImageDraw.ImageDraw, text: str, font: ImageFont.ImageFont, max_width_px: int):
    """
    Zawija tekst do wielu wierszy po pikselach, zachowując puste linie.
    """
    lines_out = []
...
def draw_multiline_centered(panel: Image.Image, text: str, fill_rgba, font_choice: str, bold: bool,
    ...
    """
    Wielowierszowy tekst wyśrodkowany (blok) + auto-dopasowanie rozmiaru.
...
def draw_spine_text_centered(canvas: Image.Image, spine_x: int, spine_w: int, canvas_h: int,
    ...
    """
    Tekst na grzbiecie:
    - idealnie wyśrodkowany (uwzględnia bbox offsety fontu)
    - rotation = -90 (odwrócony "w drugą stronę")
    """
    text = (text or "").strip()
    if not text:
        return
    ...
    # KLUCZ: korekta bbox[0], bbox[1] -> prawdziwe centrowanie
...
# APP
# =========================
st.set_page_config(page_title="Blu-ray Cover Builder", layout="wide")
st.title("Generator okładek Blu-ray (270×150 mm)")
st.caption("Prosto: wgraj pliki → ustaw grzbiet → opcjonalnie ustaw tył i logo → pobierz JPG.")

# ---- defaults (Twoje wymagania) ----
defaults = {
    "swap": False,

    # GRZBIET: domyślnie pogrubiony i 90
...
# =========================
# LAYOUT: lewa konfiguracja, prawa podglad
# =========================
...
with left:
    ...
    with tabs[0]:
        st.subheader("Pliki")
        front_file = st.file_uploader("Przód", type=["png", "jpg", "jpeg", "webp"], key="u_front")
        back_file = st.file_uploader("Tył (opcjonalnie)", type=["png", "jpg", "jpeg", "webp"], key="u_back")
        st.checkbox("Zamień przód z tyłem", key="swap")

        st.caption("Uwaga: jeśli wgrasz plik TYŁ, aplikacja automatycznie użyje go zamiast białego tła z tekstem.")

    # Auto: jeśli użytkownik wgra plik TYŁ -> przełącz na "Obraz"
    ...
    with tabs[4]:
        ...
        st.caption("Użyj tylko, jeśli kadr jest niepoprawny. W większości przypadków zostaw 0%.")
